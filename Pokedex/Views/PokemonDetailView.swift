...
struct PokemonDetailView: View {
    ...
    @State var colorFondo: LinearGradient = LinearGradient(
        colors: [.white], startPoint: .top, endPoint: .bottom)
    ...
    var body: some View {
        VStack(spacing: -50) {
            if isLoading {
                ...
                CabeceraConNombre(
                    nombre: pokemon.name.capitalizedFirstLetter(),
                    id: String(format: "%04d", pokemon.id)
                ...
                ScrollView {
                    ZStack {
                        VStack(spacing: 30) {
                            ...
                            ]).cornerRadius(48).frame(height: 100).padding()

                            //Botones de extrella y boton altavoz
                            Spacer(minLength: 45)
                            ZStack {
                                Rectangle().cornerRadius(25).foregroundColor(
                                    .white
                            ...
                            ZStack {
                                ...
                                VStack {
                                    Text("Habilidades").font(.title).frame(
                                        height: 60)
                    ...
                    .ignoresSafeArea().cornerRadius(48)
            }
        }.onAppear {
            ...
            if pokemon.types.count == 2 {
                colorFondo = PokemonType.getGradient(
                    for: [
                        pokemon.types[0].type.name, pokemon.types[1].type.name,
                    ], firstColorProportion: 1)
            } else {
                colorFondo = PokemonType.getGradient(for: [
            ...
            pokemonViewModel.fetchPokemonSpecies(id: pokemon.id) { result in
                ...
                case .success(let details):
                    ...
                    if let evolutionURL = details.evolutionChain?.url {
                        group.enter()
                        var pokemonEvolutionID = evolutionURL.split(
                            separator: "/"
                        ).last
                        pokemonViewModel.fetchPokemonEvolutionChain(
                            id: String(pokemonEvolutionID ?? "-1")
                        ) { result in
                            defer { group.leave() }
                            switch result {
                            case .success(let details):
                                ...
                                func processChain(_ chain: Chain?) {
                                    ...
                                    if let pokemonName = chain.species?.name {
                                        group.enter()
                                        pokemonViewModel.fetchPokemonDetails(
                                            id: pokemonName
                                        ) { result in
                                            defer { group.leave() }
                                            switch result {
                                            ...
                                            case .failure(let error):
                                                print(
                                                    "Error al obtener el Pokémon \(pokemonName): \(error)"
                                                )
                                            }
                                        }
                                    }
                            ...
                            case .failure(let error):
                                print(
                                    "Error al obtener detalles de la cadena evolutiva: \(error)"
                                )
                            }
                        }
                    } else {
                        print(
                            "Error: URL de cadena de evolución no encontrada en pokemonSpecies"
                        )
                    }
                case .failure(let error):
                    print(
                        "Error al obtener detalles del Pokémon \(pokemon.id): \(error)"
                    )
                }
            }
            ...
            for ability in pokemon.abilities {
                group.enter()
                pokemonViewModel.fetchAbilityInfo(name: ability.ability.name) {
                    result in
                    defer { group.leave() }
                    switch result {
                    case .success(let details):
                    ...
                    case .failure(let error):
                        print(
                            "Error al obtener habilidades del Pokémon \(pokemon.id): \(error)"
                        )
                    }
                }
            }
...
struct StatHexagonView: View {
    ...
    init(
        stats: [StatValue], size: CGFloat = 200, strokeColor: Color = .green,
        fillColor: Color = Color.gray.opacity(0.2)
    ) {
        self.stats = stats
        self.size = size
    ...
    var body: some View {
            ...
            TabView(selection: $selectedImageIndex) {
                ...
                ForEach(0..<(imageNames?.count ?? 0), id: \.self) { index in
                    ZStack(alignment: .topLeading) {
                        ...
                        if let imageName = imageNames?[index],
            ...
            HStack {
                ForEach(0..<(imageNames?.count ?? 0), id: \.self) { index in
                    ...
                    Capsule()
                        .fill(
                            Color.white.opacity(
                                selectedImageIndex == index ? 1 : 0.33)
                        )
        ...
        .onReceive(timer) { _ in
            ...
            withAnimation(.default) {
                selectedImageIndex =
                    (selectedImageIndex + 1) % (imageNames?.count ?? 0)
            }
        }
    }
}
...
class AudioPlayer: NSObject, ObservableObject {
    ...
    func playSound(for pokemonName: String) {
        ...
        let formattedName = pokemonName.lowercased().replacingOccurrences(
            of: " ", with: "")
        guard
            let url = URL(
                string:
                    "https://play.pokemonshowdown.com/audio/cries/\(formattedName).mp3"
            )
        else {
    ...
    override func observeValue(
        ...
        if keyPath == #keyPath(AVPlayerItem.status) {
            ...
            case .failed:
                print(
                    "Player item failed with error: \(String(describing: playerItem?.error))"
                )
